...
/*   Updated: 2025/06/21 10:41:25 by mszymcza         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_tile	*read_map(char *file_in, int *width, int *height)
{
	int		fd;
	char	c;
	t_tile	*map;
...
int	validate_map(t_tile *map, int width, int height)
{
	...
	for (int y = 0; y < height; y++)
	{
		...
		{
			t_tile tile = map[y * width + x];
			if ((y == 0 || y == height-1 || x == 0 || x == width-1) && tile != TILE_WALL)
				return (0);
			if (tile == TILE_START)
				player++;
